...
struct studentoStruct
{
    ...
    int pazymiuSkaicius=100;
    int* ndRez = new int[pazymiuSkaicius];
    int egzamRez;
};
...
void nd(studentoStruct* studentas, int i)
{
    ...
    while (str1.length()!=0)
    {
        ...
        {
            pazymys = stoi(str1);
            studentas[i].ndRez[kartai]=pazymys;
            kartai++;
        }
    }    
...
{   
    sort(studentas[i].ndRez, studentas[i].ndRez + studentas[i].pazymiuSkaicius);
